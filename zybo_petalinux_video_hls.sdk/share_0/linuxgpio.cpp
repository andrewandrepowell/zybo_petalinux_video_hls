...
linuxgpio::linuxgpio( uint32_t base ) : base ( base )
{
	...
	throw std::runtime_error( "Do not use this class linuxgpio! There's a high chance it doesn't even work!" );
...
void linuxgpio::set_value( uint32_t value )
{
	...
	{
		uint32_t curr_pin_1 = base+curr_pin_0;
		sprintf( val_file, "%s/gpio%u/value", GPIO_ROOT, curr_pin_1 );
		try
		{
			open_raii val_fd( open( val_file, O_RDWR ) );
			const char* val_str = ( value & 1 ) ? "1" : "0";
			write( val_fd, val_str, ( strlen( val_str ) + 1 ) );
		}
		catch ( std::runtime_error& e )
		{
			fprintf( stderr, "Cannot open the value file for GPIO %d\n", curr_pin_1 );
			throw e;
		}
	}
}
...
uint32_t linuxgpio::get_value()
{
	...
	for ( uint32_t curr_pin_0=0; curr_pin_0<nchannel; curr_pin_0++ )
	{
		uint32_t curr_pin_1 = base+curr_pin_0;
		sprintf( val_file, "%s/gpio%u/value", GPIO_ROOT, curr_pin_1 );
		...
		value <<= 1;
		char* cptr;
		value |= static_cast<uint32_t>( strtoul( val_str, &cptr, 0 ) );
		if ( cptr == optarg )
		{
