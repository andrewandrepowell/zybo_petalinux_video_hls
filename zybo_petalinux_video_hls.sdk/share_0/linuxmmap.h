...
public:
	...
	/**
	 * @brief Does the same as linuxmmap( off_t offset, size_t len ), but with the generic user I/O driver.
	 *
	 * @param uio_id Identifier associated with uio.
	 */
	linuxmmap( int uio_id );
...
private:
	...
	uint8_t* mem;
	int fd;
	linuxmmap( linuxmmap& ref ) { }
	linuxmmap& operator=( linuxmmap& ref ) { return *this; }
};
