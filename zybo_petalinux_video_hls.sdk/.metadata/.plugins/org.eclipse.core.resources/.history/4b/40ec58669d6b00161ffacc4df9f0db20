...
#define USERIN_ENABLE_CANNY		( 0b1000 )
	...
	cout << "Running main application..." << endl;
	{
		...
		while ( true )
		{
			...
			/* Acquire process_frame. */
			{
				...
				/* PROSS: Display corners with the Harris algorithm */
				...
				/* PROSS: Determine the edges and border with the Canny algorithm. */
				else if ( USERIN_ENABLE_CANNY&userinput )
				{
					Mat gray_mat;
					cvtColor( frame_0, gray_mat, CV_BGR2GRAY );

					Mat edge_mat;
					Canny( gray_mat, edge_mat, 50, 150 );

					Mat norm_mat;
					normalize( edge_mat, norm_mat, 0.0, 255.0, NORM_MINMAX );
